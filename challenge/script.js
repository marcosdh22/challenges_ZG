 ...
 */
function clearAll() {
  const losDiv = document.querySelectorAll('.contenido');
  for ( let i = 0 ; i < losDiv.length ; i++ ){
    //if (losDiv[i].hasChildNodes())
    while (losDiv[i].firstChild)
    losDiv[i].removeChild(losDiv[i].firstChild);
  }
}
...
function showMeme() {
  ...
  const elSource = document.createAttribute('src');
  laImg.src = memes;
  document.getElementById('elMeme').appendChild(laImg);
}
...
function showJoke() {
  ...
  jokeNode.textContent = randomJokeText;
  document.getElementById('laBroma').appendChild(jokeNode);
}
...
function showQuote() {
  ...
  const textQuote = randomQuote['quote'];
  const textAuthor = randomQuote['author'];
  const laCita = document.createElement('p');
  const elAutor = document.createElement('p');
  laCita.textContent = textQuote;
  elAutor.textContent = textAuthor;
  document.getElementById('elConsejo').appendChild(laCita);
  document.getElementById('elConsejo').appendChild(elAutor);
}
...
function showRiddle() {
  ...
  const elAcert = document.createElement('p');
  const laResp = document.createElement('p');
  elAcert.textContent = randomRiddle['question'];
  laResp.textContent = randomRiddle['answer'];
 // const elDisplay = document.createAttribute('display');
  document.getElementById('elAcertijo').appendChild(elAcert);
  document.getElementById('laRespuesta').appendChild(laResp);
  document.getElementById('laRespuesta').style.display = 'none';
 // elDisplay.value('none');
}
...
function revealAnswers() {
  if ( document.getElementById('elAcertijo').hasChildNodes() ){
  document.getElementById('laRespuesta').style.display = 'inline';
  window.alert('The answer has been revealed!');
  }
  else
